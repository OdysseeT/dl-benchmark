...
class COCOJoints(object):
    def __init__(self):
        ...
        for mpi, stage in zip([self.mpi, self.test_mpi], ['train', 'val']):
            if stage == 'train':
                self._train_gt_path=os.path.join(cur_dir, 'person_keypoints_trainvalminusminival2014.json')
                coco = COCO(self._train_gt_path)
            else:
                self._val_gt_path=os.path.join(cur_dir, 'person_keypoints_minival2014.json')
                coco = COCO(self._val_gt_path)
            if stage == 'train':
                for aid in list(coco.anns.keys()):
                    ann = coco.anns[aid]
