...
class ModelDesc(object, metaclass=abc.ABCMeta):
    def __init__(self):
        self._loss = None
        self._inputs = []
        self._outputs = []
    ...
    def train(self):
        ...
        for itr in range(start_itr, self.cfg.max_itr + nr_itrs, nr_itrs):
            ...
            _, self.lr_eval, *summary_res = self.sess.run(
                [self.graph_ops[0], self.lr, *list(self.summary_dict.values())], feed_dict=feed_dict)
            self.timer.toc()
            ...
            screen = [
                ...
                ' '.join(['%s: %.4f' % (x[0], x[1]) for x in list(iter_summary.items())]),
            ]
...
class Tester(Base):
    ...
    def predict_one(self, data=None):
        ...
        self.timer.tic()
        res = self.sess.run([*self.graph_ops, *list(self.summary_dict.values())], feed_dict=feed_dict)
        self.timer.toc()
