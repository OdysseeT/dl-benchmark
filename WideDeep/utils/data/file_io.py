...
def _shard_dict_to_examples(shard_dict):
  ...
  n = [i for i in list(shard_dict.values())][0].shape[0]
  feature_list = [{} for _ in range(n)]
  for column, values in list(shard_dict.items()):
...
def _serialize_shards(df_shards, columns, pool, writer):
  ...
  for inp in map_inputs:
    ...
    assert len(set([v.shape[0] for v in list(inp.values())])) == 1
    for val in list(inp.values()):
